...
> 状态：开发中。开发分支 `codex/map-v2`。本方案以不修改 V1 正式配置和公开协议为首要约束。
...
- `main` 静态构建通过 `VITE_MAP_API_BASE_URL` 读取同一公开 API。
- 单元测试、类型检查、生产构建和本地 Worker 页面联调。

上线前待完成：

- 在独立 Staging 中用真实图片完成困难/噩梦数据录入与验收。
- 为后台补 V2 版本历史、预览/恢复界面（对应 API 已完成）。
- 如要求“首次打开即完全离线”，生成并验证 V2 静态构建快照。
- 微信小程序按公开契约另立项目开发；本次只保证其只读接口可直接消费。
- 生产只部署 Backend Expand，创建 `config:v2:current` 后观察；通过验收再切换默认前台入口。
...
- [ ] 确认困难前台只显示侧门；噩梦显示正门、二楼门和正确的第三级筛选。
...
- [ ] 在隐藏 V2 路由验收困难/噩梦、入口切换、第三级筛选、全图默认和地下室。
...
- [ ] 单独发布“默认前台进入 V2”的 Worker Web 版本，不与数据迁移放在同一次变更中。
- [ ] 保留 V1 路由和配置作为观察期回退入口。
- [ ] 更新 `main` 静态网页的正式 V2 API 地址并验证跨域读取；微信小程序不作为本次网页上线阻塞项。
- [ ] 记录最终 Worker Version、V2 dataVersion、配置指纹和发布时间。

### E. 上线后观察与回滚

- [ ] 上线后立即抽查首页、13 张噩梦地图、困难侧门和随机图片加载。
- [ ] 观察 Worker 错误、V2 API 状态、R2 404、前端路由和缓存问题。
- [ ] 发现 Web 问题时回滚 Worker Version；发现 V2 数据问题时只恢复 `backups-v2/`。
- [ ] 观察期内不删除 V1 KV、V1 图片、V1 备份、V2 预览环境或旧 Worker Version。
...
- 当前正式 Worker：`5303c7ea-f896-4922-8765-aedb110a9a22`。
...
- 下一阶段仍未执行：把 `/` 默认入口从 V1 切换到 V2。该切换必须单独发布，验收后再进行。
