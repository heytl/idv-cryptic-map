...
export const REQUIRED_LAYOUTS: Record<GameMode, readonly FloorType[]> = {
  hard: ["full", "floor1", "floor2"],
  nightmare: FLOOR_ORDER,
};

export const REQUIRED_ENTRANCES: Record<GameMode, readonly EntranceType[]> = {
  hard: ENTRANCE_TYPES,
  nightmare: ["front", "upstairs"],
};

export const ENABLED_ENTRANCES: Record<GameMode, readonly EntranceType[]> = {
  hard: ["side"],
  nightmare: ["front", "upstairs"],
};
...
export function publicationIssues(map: MapItemV2): string[] {
  ...
  for (const type of requiredTypes) {
    ...
    if (!entrance.direction)
      issues.push(`${ENTRANCE_LABELS[type]}还没有选择方向`);
  ...
  for (const entrance of map.entrances) {
    if (!requiredTypes.includes(entrance.type)) {
      issues.push(
        `${MODE_LABELS[map.mode]}模式不使用${ENTRANCE_LABELS[entrance.type]}，请先移除`,
      );
    }
  }
  return issues;
...
const ASSET_PREFIXES: Record<
  "full" | "basement" | "floor1" | "floor2" | "entrance" | "thumb",
  readonly string[]
> = {
...
function validateAsset(
  asset: unknown,
  role: keyof typeof ASSET_PREFIXES,
  path: string,
  errors: string[],
): void {
...
export function validateMapConfigV2(input: unknown): ValidationResult {
  ...
  if (typeof config.updatedAt !== "string")
    errors.push("updatedAt 必须是字符串");
  if (!Array.isArray(config.maps))
    return { valid: false, errors: [...errors, "maps 必须是数组"] };

  const mapIds = new Set<number>();
  const entranceIds = new Set<string>();
  for (const map of config.maps) {
    const mapPath = `map[${String(map?.id)}]`;
    if (!Number.isInteger(map?.id) || mapIds.has(map.id))
      errors.push(`${mapPath}.id 缺失或重复`);
    else mapIds.add(map.id);
    if (!isOneOf(map?.mode, GAME_MODES)) errors.push(`${mapPath}.mode 不合法`);
    if (typeof map?.name !== "string" || !map.name.trim())
      errors.push(`${mapPath}.name 缺失`);
    if (typeof map?.displayName !== "string" || !map.displayName.trim())
      errors.push(`${mapPath}.displayName 缺失`);
    if (typeof map?.remarks !== "string")
      errors.push(`${mapPath}.remarks 必须是字符串`);
    if (!Number.isFinite(map?.sort)) errors.push(`${mapPath}.sort 必须是数字`);
    if (typeof map?.published !== "boolean")
      errors.push(`${mapPath}.published 必须是布尔值`);
    if (!map?.layout || typeof map.layout !== "object")
      errors.push(`${mapPath}.layout 必须是对象`);
    ...
    for (const floor of FLOOR_ORDER) {
      ...
      if (asset)
        validateAsset(asset, floor, `${mapPath}.layout.${floor}`, errors);
    }
    ...
    for (const entrance of map.entrances) {
      ...
      if (
        entrance?.direction !== undefined &&
        !isOneOf(entrance.direction, DIRECTIONS_V2)
      ) {
        errors.push(`${entrancePath}.direction 不合法`);
      }
      if (
        entrance?.passage !== undefined &&
        !isOneOf(entrance.passage, PASSAGES_V2)
      ) {
      ...
      if (entrance?.thumb)
        validateAsset(entrance.thumb, "thumb", `${entrancePath}.thumb`, errors);
    }

    if (!map.published || !isOneOf(map.mode, GAME_MODES)) continue;

    errors.push(
      ...publicationIssues(map).map((issue) => `${mapPath}：${issue}`),
    );
  }
...
export function toPublicMapConfigV2(
  config: MapConfigV2,
  mediaBaseUrl: string,
): PublicMapConfigV2 {
  return {
    ...
    dictionaries: {
      ...
      entrances: ENTRANCE_TYPES.map((value) => ({
        value,
        label: ENTRANCE_LABELS[value],
      })),
      directions: DIRECTIONS_V2.map((value) => ({
        value,
        label: DIRECTION_LABELS[value],
      })),
      passages: PASSAGES_V2.map((value) => ({
        value,
        ...
        return {
          ...
          entrances: map.entrances.map((entrance) => {
            if (!entrance.direction || !entrance.image) {
              throw new Error(
                `已发布地图 ${map.id} 的入口 ${entrance.id} 不完整`,
              );
            }
            // 正门的物理入口固定在南侧；前台第三级改按 passage 分类。
            const direction =
              entrance.type === "front" ? "south" : entrance.direction;
            return {
              id: entrance.id,
              type: entrance.type,
              ...
              ...(entrance.passage
                ? {
                    passage: entrance.passage,
                    passageLabel: PASSAGE_LABELS[entrance.passage],
                  }
                : {}),
              imageUrl: assetUrl(mediaBaseUrl, entrance.image),
              thumbUrl: assetUrl(
                mediaBaseUrl,
                entrance.thumb ?? entrance.image,
              ),
            };
          }),
        };
