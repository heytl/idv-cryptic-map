...
const SOURCES: readonly SourceSpec[] = [
  ...
  { file: '北 - 仙人掌门 路线.jpg', name: '北-仙人掌门', direction: 'north', floor1Top: 640, basementTop: 1510, footerTop: 2200 },
  { file: '北 - 回门 路线.jpg', name: '北-回门', direction: 'north', floor1Top: 820, basementTop: 1630, footerTop: 2200 },
...
if (!inputDirArg) {
  console.error('用法: pnpm import:nightmare -- --input-dir=<单人模式地图目录> [--output-dir=<检查目录>] [--api-url=<V2地图接口> --image-api-url=<V2图片接口> --commit]');
  process.exit(2);
}
...
if (commit && (!mapsApiUrl || !imageApiUrl)) {
  console.error('--commit 必须同时提供两个 V2 接口地址。');
  process.exit(2);
}
...
const missingFiles = SOURCES.filter((item) => !availableFiles.has(item.file));
...
async function upload(kind: 'full' | 'basement' | 'floor1' | 'floor2', buffer: Buffer): Promise<AssetRef> {
...
console.log(`正在生成 ${SOURCES.length} 张噩梦地图的全图和三层图……`);
...
const current = await currentResponse.json() as MapConfigV2;
const duplicates = SOURCES.filter((spec) => current.maps.some((map) => map.mode === 'nightmare' && map.name === spec.name));
...
for (const item of prepared) {
  const [full, basement, floor1, floor2] = await Promise.all([
    upload('full', item.full),
    upload('basement', item.basement),
    upload('floor1', item.floor1),
    upload('floor2', item.floor2),
  ]);
...
const saved = await writeResponse.json() as { version?: number; updatedAt?: string };

writeFileSync(join(outputDir, 'import-result.json'), `${JSON.stringify({
  previousVersion: current.version,
  savedVersion: saved.version,
  maps: importedMaps.map((map) => ({ id: map.id, name: map.name, direction: map.entrances[0]?.direction, published: map.published })),
  uploadedKeys,
}, null, 2)}\n`);
console.log(`完成：已新增 ${importedMaps.length} 张噩梦地图草稿，V2 配置为 v${saved.version ?? '?'}。`);
